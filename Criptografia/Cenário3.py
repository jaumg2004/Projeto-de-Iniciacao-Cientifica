...
class BaixoRuidoCanalRayleigh(CenárioBase, Plotagem, Hamming):
    ...
    def cenario(self, x, h1, h2, nBits, plot, size, tabela):
        ...
        for i in range(self.ntestes):
            ...
            y1 = self.calculaY(x[0], h1[0], self.variancia - 1.3, self.media, nBits)
            print('y1 =', y1)
            erros_y1 = self.encontraErros(x[0], y1)
            print('Erros do y1 =', erros_y1)

            y2 = self.calculaY(x[0], h2[0], self.variancia - 1.3, self.media, nBits)
            print('y2 =', y2)
